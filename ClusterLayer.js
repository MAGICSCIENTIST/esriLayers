  ...
  PopupTemplate, GraphicsLayer
    ...
    return declare([GraphicsLayer], {
      ...
      _setMap: function (map, surface) {
        ...
        this._clusterGraphics(this._clusterData);
        ...
        // connect to onPanEnd so data inSight is re-clustered when PanEnd
        this.refresh = connect.connect(map, "onExtentChange", this, function () {
          debugger
          if (!this._isExtentChange()) {
            return
          }
          this._refreshExtent();
        })
        ...
        this.lastExtent = this._map.extent;
        return div;
      },
      ...
      _unsetMap: function () {
        ...
        connect.disconnect(this.refresh);
      },
      ...
      onClick: function (e) {
        ...
        if (singles.length > this._maxSingles) {
          if (typeof swal === 'function') {
            swal("抱歉", "选取的集合点超过了 " + this._maxSingles + " 个. 请放大来获取更详细信息.", "info")
          } else {
            // alert("Sorry, that cluster contains more than " + this._maxSingles + " points. Zoom in for more detail.");
            alert("抱歉, 选取的集合点超过了 " + this._maxSingles + " 个. 请放大来获取更详细信息.");
          }
          return;
        } else {
      ...
      IsinSight: function (p, extent) {
        if (!p) {
          return false
        }
        var c = p.type;
        var _extent = extent || this._map.extent;
        if ("point" === c || "extent" === c) {
          return _extent.contains(p);
        }
        return p.x && p.y && _extent.xmin <= p.x && p.x <= _extent.xmax && _extent.ymin <= p.y && p.y <= _extent.ymax
      },
      //Remove some clusters and data points.  
      _remove: function (graphic) {
        if (graphic instanceof Array) {
          for (let i = 0; i < graphic.length; i++) {
            this.remove(graphic[i]);
          }
          this._clusters = this._clusters.filter(function (x) {
            /// if x is in graphic array.
            return graphic.findIndex(function (g) { return g.attributes.clusterId === x.attributes.clusterId }) === -1;
          });
        } else {
          this.remove(arguments);
          var index = this._clusters.findIndex(function (x) { return x.attributes.clusterId });
          this._clusters.splice(index, index + 1);
        }
      },

      // internal methods   
      _clusterGraphics: function (data) {
        var _clusterData = data
        // first time through, loop through the points  
        for (var j = 0, jl = _clusterData.length; j < jl; j++) {
          // see if the current feature should be added to a cluster  
          var point = _clusterData[j];
          var clustered = false;
      ...
      //fix the bug that click event can not fire ...
      _isExtentChange: function () {
        var curextent = this._map.extent;
        if (!this.lastExtent) {
          this.lastExtent = curextent;
          return true;
        }

        return curextent.xmin !== this.lastExtent.xmin || curextent.xmax !== this.lastExtent.xmax
          && curextent.ymin !== this.lastExtent.ymin || curextent.ymax !== this.lastExtent.ymax
      },

      //regenerate cluster in curent extent
      _refreshExtent:function(){
        this._clusterResolution = this._map.extent.getWidth() / this._map.width;
        var self = this;
        var extent = this._map.extent;
        this.lastExtent = extent;
        var inSightG = this.graphics.filter(function (x) {
          return self.IsinSight(x.geometry, extent);
          // return extent.contains(x.geometry);
        })
        var insightC = this._clusterData.filter(function (x) {
          return self.IsinSight(x, extent)
        })
        this._remove(inSightG);       
        this._clusterGraphics(insightC);
      }
